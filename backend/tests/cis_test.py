...
def collector(indexer_client):
    return CisBenchmarkCollector(indexer_client=indexer_client)
...
class TestCisBenchmarkCollectorPerformance:

    def test_perf_score_global(self, collector):
        kpi, ms = run_timed(
            "get_score_global",
            collector.get_score_global, DATE_FROM, DATE_TO,
        )
        assert kpi is not None
        assert ms < ERROR_THRESHOLD_MS, f"Requete trop lente : {ms:.0f} ms"

    def test_perf_score_by_policy(self, collector):
        kpi, ms = run_timed(
            "get_score_by_policy",
            collector.get_score_by_policy, DATE_FROM, DATE_TO,
        )
        assert kpi is not None
        assert ms < ERROR_THRESHOLD_MS, f"Requete trop lente : {ms:.0f} ms"
    
    def test_perf_agents_scanned(self, collector):
        kpi, ms = run_timed(
            "get_agents_scanned",
            collector.agents_scanned, DATE_FROM, DATE_TO,
        )
        assert kpi is not None
        assert ms < ERROR_THRESHOLD_MS, f"Requete trop lente : {ms:.0f} ms"
    ...
    def test_perf_all_sequential(self, collector):
        ...
        """
        queries = {
            "score_global":            collector.get_score_global,
            "score_by_policy": collector.get_score_by_policy,
            "agents_scanned" : collector.agents_scanned,
        }
